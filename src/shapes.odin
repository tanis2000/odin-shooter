...
sh_draw_triangle :: proc(v0: [2]f32, v1: [2]f32, v2: [2]f32) {
	opts := b_default_batcher_texture_options()
	b_tri(&state.shapes_batcher, v0, v1, v2, 0, 0, state.pixel_tex, opts)
}
...
sh_draw_circle :: proc(center: [2]f32, radius: f32) {
	...
	theta: f32 = 0.0
	v0: [2]f32 = {center.x, center.y}
	for i := 0; i < circle_segments; i += 1 {
		v1: [2]f32 = {
			linalg.cos(theta + increment) * radius + center.x,
			linalg.sin(theta + increment) * radius + center.y,
		}
		v2: [2]f32 = {linalg.cos(theta) * radius + center.x, linalg.sin(theta) * radius + center.y}
